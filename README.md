...
A local-first PDF reader with two ways to read with LLMs:
...
- **Chapter summaries** — drop a PDF, get a standalone interactive HTML
  site for the chapter — one section per concept, web-component
  figures, 3D models, KaTeX math, flashcards. [**Live demo →**][demo]
...
PDFs and threads live in your browser's IndexedDB. No server-side
database. The dev server only proxies API calls and serves static files.

## Threads

- **Multiple models per thread** — invite different agents into the
  same conversation by mentioning them.
- **Tools** — the agents can search inside the paper, fetch arXiv
  metadata, read web pages, run Python in a sandbox, scroll the viewer,
  and highlight passages for you.
- **`@code`** — routes through the [Claude Agent SDK][sdk] with full
  Read/Edit/Bash/Grep/WebFetch tools, in a per-thread sandbox directory.

[sdk]: https://www.npmjs.com/package/@anthropic-ai/claude-agent-sdk

## Chapter summaries

The agent reads the chapter, plans sections, then dispatches one writer
per section in parallel. Each writer produces a self-contained HTML
fragment using a small `pc-*` web-component library (`pc-3d`,
`pc-equation`, `pc-stepped`, `pc-anki`, `pc-grid`, …). Output is plain
HTML and runs without paperchat — the [live demo][demo] is hosted on
...
Settings → pick planner + writer models, drop a PDF, click **Summarize**.
The chapter lands in `cc-workdir/chapter-<id>/`.
...
├── chapter-summary-openrouter.mjs  # chapter-summaries agent loop (OpenRouter)
├── chapter-run-child.mjs           # detached agent runner
├── _lib/                           # pc-* web component library (used by generated chapters)
├── docs/                           # GitHub Pages — live demo chapter
└── …
```
